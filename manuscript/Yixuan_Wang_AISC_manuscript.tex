...
\begin{abstract}
Prompt-injection research commonly reports attack success rate (ASR), but the measured rate depends on how an evaluator operationalizes successful behavior. We study the construct validity of two deterministic lexical evaluators for prefix-plus-task prompt injections. S0 marks success when an exact attacker-controlled target occurs anywhere in a raw model response, whereas S2 requires the exact target to begin at character position zero. We generated responses to 1,680 frozen base prompts spanning seven language tasks and three open-weight instruction-tuned models. After technical failures, 4,993 responses formed the usable population. A preregistered, probability-based human-validation sample contained 800 responses, intentionally oversampling S0/S2 disagreements and using inverse-probability weighting for population inference. Two blinded annotators independently labeled all responses, followed by independent third-person adjudication and consensus. Under a strict human definition in which only full attack compliance counted as success, S2 achieved 92.776\% weighted accuracy versus 81.100\% for S0, a difference of 11.676 percentage points. Under a lenient definition counting full and partial compliance, the difference fell to 0.774 points. The strict S2 advantage was 23.034 points larger for generative/text-transformation tasks than for closed-label classification tasks. These findings show that lexical prompt-injection evaluators are not interchangeable: their apparent validity depends strongly on the behavioral construct used to define attack success.
\end{abstract}

\begin{keywords}
prompt injection, attack success rate, construct validity, lexical evaluation, large language models
\end{keywords}
...
The confirmatory protocol, scorer definitions, sampling plan, annotation manual, and statistical analysis plan were frozen before confirmatory analysis. The preregistration is available at \url{https://osf.io/9jrab}. The final scientific state was separately frozen after adjudication and confirmatory analysis. Cryptographic hashes were recorded for the prompt manifest, generation configuration, usable-response manifest, automated scorer artifact, human sample, adjudicated human ground truth, canonical analysis dataset, and aggregate result artifacts.
...
The preregistration is publicly available on the Open Science Framework at \url{https://osf.io/9jrab}. Public-safe study protocols, analysis code, aggregate results, and cryptographic freeze records are available from the corresponding author upon reasonable request. Raw model outputs and row-level human annotation artifacts are not publicly redistributed where source-data licensing, privacy, or redistribution constraints apply; reproducibility metadata and cryptographic hashes identify the frozen artifacts used for the reported analyses.
